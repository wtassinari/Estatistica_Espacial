---
title: "Especialização em Estatística Aplicada (PPGEAp): Módulo de Estatística Espacial"
...
output:
  html_document:
    ...
    highlight: kate
    theme: readable  # ou outro tema do rmarkdown
---
...
iii) Largura maior ($2.500 m$): suaviza padrões locais, ideal para visualizar tendências gerais.
...
i) \( w_{ij} = 1 \), se \( O_i \) **toca** \( O_j \) (ou seja, compartilham uma fronteira comum). Esse também é chamado de critério de **contiguidade**.

ii) \( w_{ij} = 1 \), se a **distância entre os centróides** das regiões \( O_i \) e \( O_j \) for **menor que um limite \( h \)**. Esse é o critério baseado em **distância geográfica**.

iii) \( w_{ij} = \frac{l_{ij}}{l_i} \), onde:
...
Baixe aqui os dados que utilizaremos no curso 👉 [Dados do curso](https://drive.google.com/file/d/1dNZej8k4Of3aAzfTvAnUpgGSEBbn_vOZ/view?usp=sharing)
...
```{r, echo=T, message=FALSE, warning=FALSE, comments=NA, }
...
Renomeando as variáveis para ser semlhante ao objeto do `geobr`:
